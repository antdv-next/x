...
class SkillLoader {
  ...
  constructor(options: SkillLoaderOptions = {}) {
    this.githubOwner = options.githubOwner || "antdv-next";
                ...
                .get(res.headers.location!, redirectOptions, res2 => {
                  ...
                  res2.on("end", () => {
                    if (
                      ...
                      reject(
                        new Error(
                          `HTTP ${res2.statusCode}: ${res2.statusMessage}`,
                        ),
                      );
                    }
                  });
  ...
  async downloadSkillFromGitHub(
    ...
    try {
      ...
      const basePath =
        language === "zh"
          ? "packages/x-skill/skills-zh"
          : "packages/x-skill/skills";
  ...
  async loadSkills(version = "latest", language = "en"): Promise<Skill[]> {
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        timeoutId = setTimeout(
          () => reject(new Error("List skills timeout: 60s")),
          60000,
        );
      });
      ...
      for (const skillName of skillDirs) {
        try {
          const skillTempDir = await this.downloadSkillFromGitHub(
            skillName,
            version,
            language,
          );
          ...
          console.warn(
            `Skipping skill ${skillName}: ${(error as Error).message}`,
          );
        }
      }
      ...
      console.warn(
        `Failed to load skills from remote: ${(error as Error).message}`,
      );
  ...
  async loadLocalSkills(language = "zh"): Promise<Skill[]> {
    ...
    const skillsDir =
      language === "zh"
        ? path.join(packageDir, "skills-zh")
