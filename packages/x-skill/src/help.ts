...
class HelpManager {
  ...
  showHelp(): void {
    const title =
      this.language === "zh" ? "X-Skill 安装器" : "X-Skill Installer";
    const description =
      this.language === "zh"
        ? "从GitHub获取skill文件"
    ...
    const helpDesc =
      this.language === "zh" ? "显示帮助信息" : "Show help information";
    const versionDesc =
      this.language === "zh" ? "显示程序版本号" : "Show program version";
    const example1 =
      this.language === "zh" ? "# 使用latest版本" : "# Use latest version";
    const example2 =
      this.language === "zh" ? "# 使用指定版本" : "# Use specific version";
    const example3 =
      this.language === "zh" ? "# 列出所有版本" : "# List all versions";
    const example4 =
      this.language === "zh" ? "# 显示程序版本号" : "# Show program version";
    const example5 =
      this.language === "zh"
        ? "# 使用token避免速率限制"
        : "# Use token to avoid rate limits";
