...
class SkillInstaller {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      switch (arg) {
        ...
        default:
          if (arg.startsWith("-")) {
            console.error(
              `${getMessage("unknownOption", defaultLanguage)} '${arg}'`,
            );
  ...
  handleNonInteractiveCommands(): boolean {
    ...
    if (this.args.listVersions) {
      void this.listVersions().then(() => process.exit(0));
      return true;
    }
    ...
    try {
      const spinner = ora(
        this.helpManager.colorize(
          getMessage("fetchingVersions", this.language),
          "cyan",
        ),
      ).start();
      ...
      versions.forEach((version, index) => {
        const marker =
          index === 0 ? getMessage("latestMarker", this.language) : "";
        console.log(
          `  ${this.helpManager.colorize(version, "yellow")}${marker}`,
        );
      });
    } catch (error) {
      console.error(
  ...
  async loadSkills(): Promise<void> {
    try {
      ...
      // 缓存结果
      this.setCache(
        `skills_${skills[0]?.version || version}`,
        this.skills,
        1800,
      ); // 缓存30分钟
    } catch (error) {
  ...
  async askQuestion(
    ...
    console.log(
      this.helpManager.colorize(
        `💡 ${getMessage("inputNumberTip", this.language)}`,
        "dim",
      ),
    );
  ...
  async askMultipleChoice(
    ...
    console.log(
      this.helpManager.colorize(
        `💡 ${getMessage("inputMultipleTip", this.language)}`,
        "dim",
      ),
    );
    ...
    while (attempts < maxAttempts) {
      try {
        const answer = await this.questionAsync(
          this.helpManager.colorize("➤ ", "green") +
            getMessage("pleaseSelect", this.language),
        );
        ...
        if (selected.length > 0) {
          ...
          selected.forEach(item => {
            console.log(
              `   ${this.helpManager.colorize(`• ${item}`, "green")}`,
            );
          });
          return selected;
        }
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const isGlobal =
        selectedInstallType === getMessage("globalInstall", this.language);

      // Installation process
      const totalSteps =
        selectedSoftwareList.length * selectedSkillNames.length;
  ...
  async installSkills(
    ...
    const targetPath = isGlobal
      ? targetConfig.paths.global
      : targetConfig.paths.project;
