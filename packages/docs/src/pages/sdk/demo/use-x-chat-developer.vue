...
const locales: Record<"zh-CN" | "en-US", DemoLocale> = {
  ...
  "en-US": {
    ...
    modifiedSystemPrompt: "Modified system prompt",
  },
};

// 用本地 locale hook 驱动 demo 文案和默认 system prompt，切换语言时能同步更新。
...
watch(
  ...
  localeText => {
    setMessage("developer", {
      message: {
        role: "developer",
        content: localeText.systemPrompt,
      },
    });
    setMessage("0", {
      message: {
...
const currentSystemPrompt = computed(
  () =>
    messages.value.find(message => message.message.role === "developer")
      ?.message.content || demoLocale.value.none,
);
...
function editDeveloper() {
  setMessage("developer", {
    message: {
      role: "developer",
      content: demoLocale.value.modifiedSystemPrompt,
    },
  });
}
...
<template>
  <Flex vertical gap="middle">
    <Flex vertical gap="middle">
      ...
      <div>{{ demoLocale.currentSystemPrompt }} {{ currentSystemPrompt }}</div>
      ...
      <Flex wrap gap="small" align="center">
        ...
        <Button @click="editDeveloper">
          {{ demoLocale.editSystemPrompt }}
        </Button>
      </Flex>
    </Flex>
    ...
    <BubbleList style="height: 320px" :role="role" :items="items" />
...
<docs lang="zh-CN">
对齐 antdx `developer.tsx`：通过一条 `developer` 消息维护系统提示词，并单独展示、编辑该提示词。
</docs>

<docs lang="en-US">
Aligned with antdx `developer.tsx`: keep the system prompt as a `developer` message, with dedicated display and edit controls.
</docs>
