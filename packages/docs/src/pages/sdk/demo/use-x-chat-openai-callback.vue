...
const locales: Record<"zh-CN" | "en-US", DemoLocale> = {
  ...
  "en-US": {
    ...
    noCallbackData: "No data available",
  },
};

// 用本地 locale hook 绑定 demo 文案，确保语言切换时 callback 面板和按钮文案能同步变化。
...
const provider = new OpenAIChatProvider({
  ...
  >("mock://openai-callback", {
    ...
    callbacks: {
      onUpdate: (_chunk, _headers, message) => {
        callbackMessage.value = `onUpdate: ${JSON.stringify(message || null)}`;
      },
      onSuccess: (_chunks, _headers, message) => {
        callbackMessage.value = `onSuccess: ${JSON.stringify(message || null)}`;
      },
      onError: (error, _errorInfo, _headers, message) => {
        callbackMessage.value = `onError: ${JSON.stringify({
          error: error.message,
...
const callbackText = computed(
  () => callbackMessage.value || demoLocale.value.noCallbackData,
);
...
<template>
  <Flex vertical gap="middle">
    <Flex vertical gap="middle">
      ...
      <div>Callback Message: {{ callbackText }}</div>
...
<docs lang="zh-CN">
对齐 antdx `openai-callback.tsx`：将 `XRequest` 的 `onUpdate / onSuccess / onError` 回调结果直接展示到页面上。
</docs>

<docs lang="en-US">
Aligned with antdx `openai-callback.tsx`: surface `XRequest` `onUpdate / onSuccess / onError` callback results directly in the UI.
</docs>
