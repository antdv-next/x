...
const RegistrationForm = {
  ...
  setup(props: any) {
    const currentStep = ref<number>(props.step ?? 0);
    ...
    const buildContext = (extra: Record<string, any>) => {
      ...
      const eventContext = props.action?.event?.context;
      if (eventContext) {
        Object.keys(eventContext).forEach(key => {
          context[key] = extra;
        });
      }
      return context;
...
const updateResultCard = (formData: any) => ({
  version: "v0.9",
  updateComponents: {
    surfaceId: "result",
    components: [
...
<docs lang="zh-CN">
使用 XCard 实现 A2UI v0.9 协议的表单验证示例。演示了多步骤注册表单，每一步都进行字段校验，提交完成后展示成功卡片。
</docs>

<docs lang="en-US">
Form validation example with XCard implementing the A2UI v0.9 protocol. Demonstrates a multi-step registration form with per-step field validation, and a success card after submission.
</docs>
