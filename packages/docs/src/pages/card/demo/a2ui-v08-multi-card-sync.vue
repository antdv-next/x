...
// v0.8: dataModel stores values as JSON strings, parse them
...
const ProductListCard = {
  ...
  setup(props: any) {
    ...
    return () => {
      const products = parseJsonString<Product[]>(props.products, allProducts);
      const cart = parseJsonString<CartItem[]>(props.cart, []);
...
const CartCard = {
  ...
  setup(props: any) {
    const handleQuantityChange = (productId: number, quantity: number) => {
      if (!props.updateAction?.name) return;
      props.onAction?.(props.updateAction.name, { productId, quantity });
    };

    const handleRemove = (productId: number) => {
      if (!props.removeAction?.name) return;
      props.onAction?.(props.removeAction.name, { productId });
    };
                      ...
                      >
                        ...
                        <SpaceCompact size="small">
                          <InputNumber
                            ...
                            style={{ width: "60px" }}
                          />
                          <Button danger onClick={() => handleRemove(item.product.id)}>
                            {h(DeleteOutlined)}
                          </Button>
                        </SpaceCompact>
                      </div>
  ...
  setup(props: any) {
    return () => {
      ...
      const handleCheckout = () => {
        if (!props.checkoutAction?.name) return;
        props.onAction?.(props.checkoutAction.name, {
          cart,
          subtotal,
          discount,
          shipping,
  ...
  setup(props: any) {
    return () => {
      ...
      const numericItemCount =
        typeof props.itemCount === "string" ? parseInt(props.itemCount, 10) : props.itemCount;
          ...
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            ...
            >
              ...
              >
                ...
                <TypographyText style={{ color: "#fff" }}>
                  {numericItemCount} items
                </TypographyText>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <TypographyText style={{ color: "rgba(255,255,255,0.7)" }}>
...
// ═══════════════════════════════════════════════════════════════════════════
const ShopSurfaceUpdateCommand = {
  surfaceUpdate: {
    surfaceId: "shop",
    components: [
      {
      ...
      },
      {
        ...
        component: {
          MultiCardContainer: {
            children: { explicitList: ["col-products", "col-cart", "col-summary"] },
          },
        },
      },
    ],
...
const ShopBeginRenderingCommand = {
  beginRendering: { surfaceId: "shop", root: "root" },
};

const InitialDataModelUpdateCommand = {
  dataModelUpdate: {
    surfaceId: "shop",
    contents: [
      { key: "products", valueString: JSON.stringify(allProducts) },
      { key: "cart", valueString: "[]" },
    ],
  },
};
...
const CheckoutSuccessBeginRenderingCommand = {
  beginRendering: { surfaceId: "checkout-success", root: "success-card" },
};
  ...
  value => {
    if (card.value.some(c => c.id === "shop")) {
      onAgentCommand({
        dataModelUpdate: {
          surfaceId: "shop",
          contents: [{ key: "cart", valueString: JSON.stringify(value) }],
        },
      });
    }
  },
