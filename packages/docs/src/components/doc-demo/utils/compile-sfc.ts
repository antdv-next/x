...
// 演示源码可能引用的依赖，全部按需懒加载并逐个容错：
// 某个模块不可用只影响其自身的编辑预览，不阻断整个编译链路
const optionalModuleLoaders: Record<string, () => Promise<any>> = {
...
async function ensureDependencies() {
  ...
  if (!modulesReady) {
    await Promise.all(
      Object.entries(optionalModuleLoaders).map(async ([name, loader]) => {
        try {
          const mod = await loader();
          modulesMap[name] = (mod as any).default || mod;
        } catch {
...
function transformCode(code: string): string {
  ...
  result = result.replace(
    ...
    (_, name, source) =>
      `const ${name} = (__modules__["${source}"] || {}).default || (__modules__["${source}"] || {});`,
  );
