...
function cssVarToTokenName(cssVar: string): string {
  ...
  let tokenName = cssVar.replace(/-([a-z0-9])/g, (_, char) =>
    char.toUpperCase(),
  );
...
const componentTokens = computed(() => {
  ...
  if (!props.component || !components[props.component]) return [];
...
function buildTokenData(tokens: string[], component?: string): TokenData[] {
...
const componentData = computed(() => {
  if (!componentTokens.value.length) return [];
  return buildTokenData(componentTokens.value, props.component);
});

const globalData = computed(() => {
  return buildTokenData(mergedGlobalTokens.value);
});
...
<template>
  <div class="component-token-table">
    ...
    <div v-if="componentTokens.length > 0">
      <div
        class="cursor-pointer relative flex items-center justify-start leading-[40px] gap-2"
        @click="componentOpen = !componentOpen"
      >
        <RightOutlined
        ...
        >
          ...
          <a-popover destroy-on-hidden :overlay-style="{ width: '400px' }">
            <template #content>
              <a-typography>
                <pre dir="ltr" style="font-size: 12px"><code
                    dir="ltr"
                  >{{ componentCode }}</code></pre>
                <a
                  class="text-[#999]"
                  :href="t('components.componentTokenTable.customizeTokenLink')"
                  target="_blank"
                  rel="noreferrer"
                >
            ...
            <template #content>
              <a-typography>
                ...
                <a
                  ...
                  :href="
                    t(
                      'components.componentTokenTable.customizeComponentTokenLink',
                    )
                  "
                  target="_blank"
                  rel="noreferrer"
