...
> 这些是本仓库真实出现过的 Bug，来自 `issues` 与 `PR` 修复历史，与上游无关。已在 `App.vue → 本项目 Bugs (7)` 提供最小复现。
...
| 自定义      | React 对照 | 输入 @ 触发插入自定义 slot 后无法删除插入之前的前一个内容                          | React 侧 ProseMirror 选区/节点合并导致前一 custom 被跳过（Vue 需对齐验证） | L6：输入 @ 自动插“自定义4” → 光标在自定义4后 Backspace 应删 @Travel Planner3 |
