...
export const XBubble = defineComponent({
  ...
  props: {
    ...
    header: {
      type: [String, Number, Object, Array, Function] as PropType<BubbleProps['header']>,
      default: undefined,
    },
    footer: {
      type: [String, Number, Object, Array, Function] as PropType<BubbleProps['footer']>,
      default: undefined,
    },
    avatar: {
      type: [String, Number, Object, Array, Function] as PropType<BubbleProps['avatar']>,
      default: undefined,
    },
    extra: {
      type: [String, Number, Object, Array, Function] as PropType<BubbleProps['extra']>,
      default: undefined,
    },
    footerPlacement: {
  ...
  setup(props, { expose }) {
    ...
    const usingInnerAnimation = computed(() => {
      return Boolean(mergedTyping.value) && typeof memoedContent.value === 'string'
    })

    watch(
      () => [memoedContent.value, usingInnerAnimation.value, props.streaming] as const,
      () => {
        if (usingInnerAnimation.value)
          return
        if (props.streaming)
          return
    ...
    const footerPlacement = computed(() => {
      if (props.footerPlacement)
        return props.footerPlacement
      return props.placement === 'start' ? 'outer-start' : 'outer-end'
    })

    const isEditing = computed(() => {
      return typeof props.editable === 'boolean' ? props.editable : props.editable?.editing
    })

    const isFooterInner = computed(() => footerPlacement.value.includes('inner'))
      ...
      return (
        <div
          class={[
            ...
            {
              [`${props.prefixCls}-footer-start`]: footerPlacement.value.includes('start'),
              [`${props.prefixCls}-footer-end`]: footerPlacement.value.includes('end'),
            },
          ]}
    ...
    const renderMainContent = () => {
      if (props.loading)
        return props.loadingRender ? props.loadingRender() : <Loading prefixCls={props.prefixCls} />

      if (isEditing.value) {
        if (typeof props.content !== 'string')
          throw new Error('[Bubble] Editable mode only supports string content.')

        return (
          <EditableContent
            prefixCls={props.prefixCls}
            content={props.content}
            okText={(props.editable as EditableBubbleOption | undefined)?.okText}
            cancelText={(props.editable as EditableBubbleOption | undefined)?.cancelText}
      ...
      if (usingInnerAnimation.value && typeof memoedContent.value === 'string') {
        return (
          <TypingContent
            prefixCls={props.prefixCls}
    ...
    return () => (
      ...
      >
        ...
        {!isEditing.value && !props.loading && renderBubbleSlot(props.extra, props.content, props.info)
          ? (
